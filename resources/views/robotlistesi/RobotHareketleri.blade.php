...
<h4 align="center" class="modalTitle" ref="robotAdi">Robot: {{$robot->adi}}</h4>
    <form action="RobotHesapHareketleri">
        <input type="hidden" value="{{$robot->id}}" id="robotId" name="id">
        <input type="hidden" value="false" name="session">
        <label class="lbl-sm " >Tarih:</label>
        <?php $dd->MakeInput("tarih1","tarih1","RobotHesap".$robot->id);?>
        - 
        <?php $dd->MakeInput("tarih2","tarih2","RobotHesap".$robot->id);?>
        <label class="lbl-sm " >İslem:</label>
        <?php $dd->Make($dd->DdIslemTuru(),"islemTuru","islemTuru","RobotHesap".$robot->id);?>
    ...
    <table align="center" border="1" bordercolor="lightblue" cellspacing="100" class="tableYeniPaketler" >
        <tbody>
            <tr style="color:white; background-color:#0b3779">
                ...
                <td style="width:70px;" align="center">Pos<br> Bakiyesi</td>
        ...
        <?php
            ...
            foreach ($hareketler as $hareket) 
            {
                ...
                $tutar=(double)$hareket->sonrakiBakiyeSistem-$hareket->oncekiBakiyeSistem;
                $sayfaTutarToplami=$sayfaTutarToplami+$tutar;
                $oncekiBakiyeSistem=number_format( $hareket->oncekiBakiyeSistem, 3, '.', '');
                $sonrakiBakiyeSistem=number_format( $hareket->sonrakiBakiyeSistem, 3, '.', '');
                $posBakiye=number_format( $hareket->posBakiye, 3, '.', '');
                ...
                <td  align='center'>$oncekiBakiyeSistem<br>$sonrakiBakiyeSistem</td>
    ...
    <?php
        ...
        { 
            ...
            echo "
            <a 
                href='RobotHesapHareketleri?sayfa=$i&$filtreler'
...
function Temizle()
{
    ...
    window.location.href = "RobotHesapHareketleri/temizle?id="+id; 
}
</script>
