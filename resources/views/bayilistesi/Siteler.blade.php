...
<div >
    ...
    <form class="form-horizontal" align="center" name="disbayiler" style=" display:inline ;clear:both; " id="disbayiler" action="bayinohareket-siteler" >
        <div class="form-group col-md-12 filter"  align="left" >
            ...
            <?php $dd->Make($dd->DdBayiler(),"bayiler","bayiler","Siteler");?>

            <label style="width:auto!important;" class="lbl-lg " >Bayi Site Adres:</label>
            <?php $dd->MakeInput("siteadres","siteadres","Siteler");?>
    ...
    <table align="center" border="1" bordercolor="lightblue" cellspacing="100" class="tableBayiListesi" >
        <tbody>
            <tr style="color:white; background-color:#0b3779">
                ...
                <td style="width:150px;" align="center">Site<br>Sorgu Blokaj</td>
                <td style="width:150px;" align="center">Site<br>Yükleme Blokaj</td>
                </tr>
        </tbody>
        ...
        <td align='center'>
        {{ $bayi->sorgu_blokeli_alt_bayi }} / {{ $bayi->toplam_alt_bayi }}
          <div class='onoffswitch' id='swy_{{$bayi->id}}'>
              <input type='checkbox' value='{{$bayi->id}}' class='onoffswitch-checkbox'
                  onClick='UpdateSiteBlokaj("blk_{{$bayi->id}}",{{$bayi->id}},"{{$bayi->takmaAd}}","{{$bayi->site_adres}}")' id='blk_{{$bayi->id}}'>
        ...
        <td align='center'>
        {{ $bayi->yukleme_blokeli_alt_bayi }} / {{ $bayi->toplam_alt_bayi }}
          <div class='onoffswitch' id='swyb_{{$bayi->id}}'>
              <input type='checkbox' value='{{$bayi->id}}' class='onoffswitch-checkbox'
                  onClick='UpdateSiteBlokajYukleme("blk_{{$bayi->id}}",{{$bayi->id}},"{{$bayi->takmaAd}}","{{$bayi->site_adres}}")' id='blkb_{{$bayi->id}}'>
    ...
    <?php
        ...
        for ($i=1; $i <=$sayfaSayisi ; $i++)
        {
            ...
            echo "
            <a
                href='bayinohareket-siteler?sayfa=$i'
...
function Temizle()
{
    ...
    window.location.href = "ajax/Siteler/temizle";
}
