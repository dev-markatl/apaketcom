...
class VodafoneBotApi
{
    ...
    public function cevapRobot(Request $request)
    {
        ...
        $id         = $request->input('id');
        $aciklama   = $request->input('hata_aciklama');
        ...
        if($istek[0]->maliyetFiyati !=$tutar)
            $this->FiyatBildirimi("firsat",$robot[0]->operator,$tutar,"adi",$istek[0]->tel,$robotAdi);
    ...
    function YeniPaketBildirimi(Request $request)
	{
        ...
        $this->FiyatBildirimi("firsat",$robot[0]->operator,$fiyat,$paket_adi,$gsmno,$robotAdi);
